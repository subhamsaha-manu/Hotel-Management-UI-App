...
export class CheckinCheckoutHandlerComponent {
  ...
  constructor(
    ...
    this.toBeUpdatedBooking = this.bookingDataService.bookings.find(
      (ele) => ele.bookingId === data.bookingId
    );
  ...
  onSubmit() {
    if (
      this.bookingCheckInCheckOutForm.get('checkOutDone').value === 'true' &&
      this.toBeUpdatedBooking.bookingStatus === 'PENDING'
    ) {
      Swal.fire(
        'Warning',
        'Cannot checkout guest as payment pending',
        'warning'
      ).then(() => this.dialogRef.close());
    } else {
      this.toBeUpdatedBooking.checkinDone = this.bookingCheckInCheckOutForm.get(
        'checkinDone'
      ).value;
      this.toBeUpdatedBooking.checkoutDone = this.bookingCheckInCheckOutForm.get(
        'checkOutDone'
      ).value;
      this.bookingDataService
        .updateData(
          this.toBeUpdatedBooking,
          this.toBeUpdatedBooking.roomDetails,
          this.toBeUpdatedBooking.paymentDetails
        )
        .subscribe((data) => {
          Swal.fire(
            'Success!!',
            'Booking record ' + data + ' modified successfully',
            'success'
          ).then(() => {
